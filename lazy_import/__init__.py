    ...
    def __getattribute__(self, attr):
        ...
        if (
            ...
            logger.debug(
                ...
                "inspection.".format(super(LazyModule, self).__getattribute__("__name__"), attr)
            )
        ...
        logger.debug("Returning value '{}'.".format(super(LazyModule, self).__getattribute__(attr)))
        return super(LazyModule, self).__getattribute__(attr)

    def __setattr__(self, attr, value):
        logger.debug(
            "Setting attr {} to value {}, in LazyModule instance "
            "of {}".format(attr, value, super(LazyModule, self).__getattribute__("__name__"))
        )
        _load_module(self)
...
class LazyCallable(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        try:
            ...
            raise AttributeError(msg.format(callable=self.cname, **self.error_strings)) from None
        ...
        while modname:
            try:
                ...
                class _LazyModule(lazy_mod_class):
                    ...
                    try:
                        _lazy_import_error_msgs["msg_callable"] = err_s.pop("msg_callable")
                    except KeyError:
                        pass
...
def lazy_callable(modname, *names, **kwargs):
    ...
    return tuple(
        _lazy_callable(modname, cname, error_strings.copy(), lazy_mod_class, lazy_call_class)
        for cname in names
    )
...
def _lazy_callable(modname, cname, error_strings, lazy_mod_class, lazy_call_class):
    ...
    if issubclass(modclass, LazyModule) and hasattr(modclass, "_lazy_import_callables"):
        modclass._lazy_import_callables.setdefault(cname, lazy_call_class(module, cname))
...
def _load_module(module):
    ...
    with _ImportLockContext():
        ...
        try:
            ...
            if isinstance(err, AttributeError) and not err.args[
                0
            ] == "'NoneType' object has no attribute 'name'":
                # Not the AttributeError we were looking for.
                raise
            msg = modclass._lazy_import_error_msgs["msg"]
            raise ImportError(msg.format(**modclass._lazy_import_error_strings)) from None
...
def _setdef(argdict, name, defaultvalue):
    ...
    if name not in argdict or argdict[name] is None:
